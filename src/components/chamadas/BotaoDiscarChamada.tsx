...
export function CallDialButton({ label, letters, onClick, className, disabled }: PropsBotaoDiscarChamada) {
  return (
    <button
      ...
      onClick={onClick}
      className={cx(
        'inline-flex w-full aspect-square h-10 flex-col items-center justify-center rounded-xl',
        'bg-card text-text',
    ...
    >
      ...
      {letters && <span className="text-[9px] leading-tight mt-0.5 font-normal text-muted">{letters}</span>}
    </button>
  )
}
